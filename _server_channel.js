...
(()=>{
	...
	WebSocketConnection.prototype.sendEvent = function(event, ...eventArgs) {
	...
	module.exports = ()=>{
		const HTTP_SERVER	= http.createServer((req,res)=>{res.writeHead(400); res.end();});
		const WS_SERVER		= new WebSocketServer({
			httpServer:HTTP_SERVER
		});
		const ACCEPTED_ORIGIN		= [];
		const SUPPORT_CHANNELS		= {};
		const SUPPORT_CHANNEL_LIST	= [];
